...
def run(mazeType, ai, trials, width, height, showplan):
  ...
  for mazeName in by_maze:
    print( '{maze: <16}'.format(maze=mazeName) + '\t' + str(mean(by_maze[mazeName]['times'])) + '\t' + str(mean(by_maze[mazeName]['lengths']))+ '\t' + str(mean(by_maze[mazeName]['nodes'])))

  print('\nMean Results by Agent:\n')
  print("\t\t\tTime\tLength\tNodes")
  for agentName in by_ai:
    print( '{agent: <16}'.format(agent=agentName) + '\t' + str(mean(by_ai[agentName]['times'])) + '\t' + str(mean(by_ai[agentName]['lengths']))+ '\t' + str(mean(by_ai[agentName]['nodes'])))

  print('\nMean Times by Agent Maze combinations:\n')
  print('{a:<16}'.format(a='') + '\t'.join(map(lambda x : '{item: <16}'.format(item=x), maze_list)))
  for agentName in by_ai:
    print( '{agent: <16}'.format(agent=agentName) + '\t'.join(map(lambda x: '{a:<16}'.format(a=mean(time_table[x][agentName])), maze_list)))

  print('\nMean Length by Agent Maze combinations:\n')
  print('{a:<16}'.format(a='') + '\t'.join(map(lambda x : '{item: <16}'.format(item=x), maze_list)))
  for agentName in by_ai:
    print( '{agent: <16}'.format(agent=agentName) + '\t'.join(map(lambda x: '{a:<16}'.format(a=mean(length_table[x][agentName])), maze_list)))

  print('\nMean Nodes Explored by Agent Maze combinations:\n')
  print('{a:<16}'.format(a='') + '\t'.join(map(lambda x : '{item: <16}'.format(item=x), maze_list)))
  for agentName in by_ai:
    print( '{agent: <16}'.format(agent=agentName) + '\t'.join(map(lambda x: '{a:<16}'.format(a=mean(nodes_table[x][agentName])), maze_list)))
